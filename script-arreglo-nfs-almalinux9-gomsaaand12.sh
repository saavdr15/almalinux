...
# 1. Configurar vsftpd para que permita el modo pasivo y desactivar el sandbox (crítico en EL9)
echo "pasv_enable=YES" >> /etc/vsftpd/vsftpd.conf
echo "pasv_min_port=30000" >> /etc/vsftpd/vsftpd.conf
echo "pasv_max_port=30100" >> /etc/vsftpd/vsftpd.conf
echo "seccomp_sandbox=NO" >> /etc/vsftpd/vsftpd.conf

# 2. Reiniciar el servicio para aplicar los cambios
systemctl restart vsftpd

# 3. Abrir los puertos en Firewalld (si está activo)
# Permitimos FTP puro, los puertos pasivos, y los servicios de NFS
